...
if exist "%NEXT_CLI%" goto run

if defined TAM_LOCAL_NODE_MODULES (
  set "NEXT_CLI=%TAM_LOCAL_NODE_MODULES%\next\dist\bin\next"
  set "NODE_MODULE_ROOT=%TAM_LOCAL_NODE_MODULES%"
  if exist "%NEXT_CLI%" goto run
)

set "NEXT_CLI=C:\LocalBuild\Products\node_modules\next\dist\bin\next"
...
call "%~dp0run-node.cmd" "%NEXT_CLI%" %*
exit /b %errorlevel%

:broken
echo Next.js is unavailable because dependencies are missing from both:
echo   1^) %~dp0..\node_modules
